...
import { Event, Disposable, EventEmitter, workspace as Workspace, Uri, FileType } from 'vscode';
...
export function log(...args: any[]): void {
  if(args[0] === 'ERROR'){
    console.error.apply(console, ['wat:', ...args]);
  } else {
    console.log.apply(console, ['wat:', ...args]);
  }
}
...
export async function mkdirp(uri:Uri): Promise<boolean> {
	const mkdir = async () => {
		try {
			...
			if (err.code === 'EEXIST') {
				...
				throw new Error(`'${uri.fsPath}' exists and is not a directory.`);
			}
	...
	// is root?
	if (uri.fsPath === dirname(uri.fsPath)) {
		return true;
	}
	...
	try {
		...
		await mkdirp(Uri.file(dirname(uri.fsPath)));
		await mkdir();
	}
